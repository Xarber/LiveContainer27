...
def clean_source_from_base(base, persistent):
    ...
    for track in CHANNELS:
        previous_channel = previous_channels.get(track, {})
        previous_releases = previous_channel.get("releases", [])
...
def upsert_stable_version(app, entry):
    ...
    versions = [
        ...
        if isinstance(version, dict)
        and version.get("version") != entry["version"]
    ]
...
def update_source():
    ...
    if IS_NIGHTLY:
        ...
        upsert_stable_version(app, sidestore_entry)
